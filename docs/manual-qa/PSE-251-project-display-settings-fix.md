...
- Evidence: `docs/manual-qa/screenshots/PSE-251-project-display-settings-fix/project-display-settings.png` (project route with Display settings entry and no placeholder/internal error visible).
- Tests: `apps/desktop/tests/renderer/projectsPage.test.tsx`; `pnpm typecheck`; full `pnpm test`.
- Status: code/test pass; manual save/persist screenshot capture timed out in packaged automation and remains P2 evidence follow-up.
