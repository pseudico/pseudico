export const workflowMetadataTriggersSql = `
...
create table if not exists workflow_definitions_next (
  ...
  name text not null,
  description text,
  trigger_type text not null default 'manual' check (trigger_type in ('manual', 'item_created', 'file_imported', 'tag_added', 'tag_removed', 'category_assigned')),
